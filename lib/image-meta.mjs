...
export function detectAiProvenance(pngChunks, exifRaw, c2paResult) {
  const indicators = [];
  ...
  if (pngChunks['novelai_comment'] || pngChunks.Comment) {
    indicators.push({ source: 'png-text', label: 'NovelAI metadata', detail: (pngChunks['novelai_comment'] || pngChunks.Comment).slice(0, 200) });
    isLikelyAiGenerated = true;
  }
  ...
  if (c2paResult) {
    ...
    const { integrityOk, signerTrusted, statusCodes, failureCodes } =
      classifyC2paValidation(c2paResult.validation_status);
    ...
    // verified = 改ざん検出なし（integrity）。signerTrusted = 発行者がトラストリスト内か。
    // 両者を分けることで「未署名検証(untrusted)」を「検証失敗(改ざん)」と誤表示しない。
    c2paSummary = {
    ...
    // claimGenerator が AI ツールかチェック
    if (claimGenerator && matchesAiTool(claimGenerator)) {
      indicators.push({ source: 'c2pa', label: 'AI tool claim generator', detail: claimGenerator });
      isLikelyAiGenerated = true;
    }

    // c2pa.actions の digitalSourceType チェック
    const actionAssertions = (activeManifest?.assertions ?? []).filter(
      (a) => a.label === 'c2pa.actions'
    );
    for (const aa of actionAssertions) {
      const actions = aa.data?.actions ?? [];
      for (const action of actions) {
        if (action.digitalSourceType === 'trainedAlgorithmicMedia') {
          indicators.push({ source: 'c2pa', label: 'C2PA: trainedAlgorithmicMedia', detail: action.digitalSourceType });
          isLikelyAiGenerated = true;
        }
      }
    }

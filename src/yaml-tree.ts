...
export function renderYamlTree(yamlStr: string): string {
  try {
...
function renderNode(value: unknown, key: string, isRoot = false): string {
  ...
  if (typeof value === 'string') return `<div class="jt-line">${keyHtml}<span class="jt-str">"${esc(value)}"</span></div>`;
  ...
  return `<div class="jt-line">${keyHtml}<span class="jt-str">${esc(String(value))}</span></div>`;
}

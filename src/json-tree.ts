...
function renderNode(value: unknown, key: string, isRoot = false, maskValue?: SecretMasker): string {
  ...
  if (typeof value === 'number') return `<div class="jt-line">${keyHtml}<span class="jt-num">${escapeHtml(maskValue ? maskValue(String(value), key) : String(value))}</span></div>`;
  if (typeof value === 'string') return `<div class="jt-line">${keyHtml}<span class="jt-str">"${escapeHtml(maskValue ? maskValue(value, key) : value)}"</span></div>`;
  ...
  return `<div class="jt-line">${keyHtml}<span class="jt-str">${escapeHtml(rendered)}</span></div>`;
}
